...
impl fmt::Display for CircuitError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self {
            CircuitError::BadInputCount => write!(f, "Error, circuit input wires are used as output."),
            CircuitError::BadOutputCount => write!(f, "Bad output count"),
            CircuitError::BadWireCount(a,b) => write!(f, "Bad wire count, actual {a}, but {b} defined"),
...
fn verify_circuit(circuit : &ArithCircuit) -> Result<(), CircuitError> {
    let num_wires = circuit.gates.iter()
        ...
        .flatten()
        .chain(
            circuit.gates.iter().map(|g| &g.output)
        )
        .unique()
        .count();
    ...
    let all_outputs : Vec<usize> = circuit.gates.iter()
        .map(|g| g.output).collect();
