...
fn bench_bitvec(c: &mut Criterion) {
    c.bench_function("XOR bitvec u8", |bench| bench.iter(|| {
...
fn bench_bitvec64(c: &mut Criterion) {
    c.bench_function("XOR bitvec u64", |bench| bench.iter(|| {
        let mut rng = thread_rng();
...
fn bench_vec_of_bytes(c: &mut Criterion) {
    c.bench_function("XOR bytes", |bench| bench.iter(|| {
        let mut rng = thread_rng();
        let a : [u8; 1024] = rng.gen();
        let b : [u8; 1024] = rng.gen();
...
fn bench_array_of_bytes(c: &mut Criterion) {
    c.bench_function("XOR array", |bench| bench.iter(|| {
        let mut rng = thread_rng();
        let a : [u8; 1024] = rng.gen();
        let b : [u8; 1024] = rng.gen();
...
fn bench_array_of_64(c: &mut Criterion) {
    c.bench_function("XOR array 64", |bench| bench.iter(|| {
        let mut rng = thread_rng();
        let a : [u64; 1024/8] = rng.gen();
        let b : [u64; 1024/8] = rng.gen();
...
fn bench_array_of_128(c: &mut Criterion) {
    c.bench_function("XOR array 128", |bench| bench.iter(|| {
        let mut rng = thread_rng();
...
fn bench_array_of_simd(c: &mut Criterion) {
    ...
    c.bench_function("XOR array simd", |bench| bench.iter(|| {
        let mut rng = thread_rng();
        let a : [u128; 1024/16] = rng.gen();
        let b : [u128; 1024/16] = rng.gen();
        black_box(xor_bytes_arr_128(&a, &b));
...
criterion_group!(benches, bench_bitvec,
    bench_bitvec64, bench_vec_of_bytes,
    bench_array_of_bytes, bench_array_of_64,
    bench_array_of_128, bench_array_of_simd);
