...
fn bench(c: &mut Criterion) {
    ...
    c.bench_function("polynomial_mul_bytes", |b| {
        b.iter(|| polynomial_mul_bytes(&mut result.0, &left, &right))
    });

    // polynomial_acc
    result.zeroize();
    c.bench_function("polynomial_acc_bytes", |b| {
        b.iter(|| polynomial_acc_bytes(&mut result.0, &left))
    });

    // polynomial_eq
    result.zeroize();
    c.bench_function("polynomial_eq_bytes", |b| {
        b.iter(|| polynomial_eq_bytes(&left, &right))
    });
